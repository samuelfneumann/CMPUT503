...
class DeadReckoningNode(DTROS):
    ...
    Subscribers:
        ...
        ~update_odom (:obj:`geometry_msgs.msg.Pose`): Set new pose
    """
    ...
    def __init__(self, node_name):
        ...
        self.parent_frame = rospy.get_param("~parent_frame").replace("~", self.veh)
        self.child_frame = rospy.get_param("~child_frame").replace("~", self.veh)
        ...
        self.timestamp = None
        self.x = rospy.get_param("~start_x", 0.0)
        self.y = rospy.get_param("~start_y", 0.0)
        self.z = rospy.get_param("~start_z", 0.0)

        self.yaw = np.deg2rad(rospy.get_param("~start_yaw", 90))
        self.q = tr.quaternion_from_euler(0, 0, self.yaw)
        self.tv = 0.0
        self.rv = 0.0

        self._pause_odom = False
        self._pause_time = DTParam(
            "~pause_time",
        ...
        self.sub_odom_update = rospy.Subscriber(
            "~update_odom", Pose, self.update_odometry,
        )
    ...
    def cb_timer(self, _):
        ...
        if not self._pause_odom:
            self.publish_odometry()
        if need_print:
            self._print_time = time.time()

    def update_odometry(self, pose: Pose):
        ...
        yaw = tr.euler_from_quaternion(q)[2]

        self.yaw = yaw

        self._pause_odom = True
        rospy.Timer(
            rospy.Duration(self._pause_time.value),
            self._continue_odom,
            oneshot=True,
        )

    def _continue_odom(self, msg):
        self._pause_odom = False
