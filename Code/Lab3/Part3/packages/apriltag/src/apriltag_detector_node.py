...
class AprilTagDetector(DTROS):
    def __init__(self):
        ...
        # # rospy.loginfo("static parameters initialized")

        # dynamic parameter
        self.detection_freq = DTParam(
            "~detection_freq", default=1, param_type=ParamType.INT,
        ...
        # rospy.loginfo("detectors initialized")

        # create a CV bridge object
        self._jpeg = TurboJPEG()

        # create subscribers
        # TODO: you should instead subscribe to ~image and then set the image
        # type
        self._img_sub = rospy.Subscriber(
            "camera_node/image/compressed", CompressedImage, self._img_cb,
            queue_size=1, buff_size="20MB"
        )
        self._cinfo_sub = rospy.Subscriber(
            "camera_node/camera_info", CameraInfo, self._cinfo_cb,
            queue_size=1,
        )
        # rospy.loginfo("camera nodes initialized")
    ...
    def _cinfo_cb(self, msg):
        ...
        with self.profiler("/cb/camera_info/get_optimal_new_camera_matrix"):
            ...
            self._camera_parameters = (
                rect_K[0, 0], rect_K[1, 1], rect_K[0, 2], rect_K[1, 2],
            )

        # create rectification map
        with self.profiler("/cb/camera_info/init_undistort_rectify_map"):
            self._mapx, self._mapy = cv2.initUndistortRectifyMap(
                self.camera_model.K, self.camera_model.D, None, rect_K, (W, H),
                cv2.CV_32FC1
            )
    ...
    def _detect(self, detector_id, msg):
        ...
        # Here, we do the actual apriltag detection
        ...
        # At this point we have a bunch of dt_apriltags.Detection objects, each
        # hold a detection of an apriltag in the scene. We now package these
        # into a DuckieTown AprilTagDetectionArray message, where each element
        # of the array is an AprilTagDetection message
        tags_msg = AprilTagDetectionArray()
        tags_msg.header.stamp = msg.header.stamp
        tags_msg.header.frame_id = msg.header.frame_id  # Camera frame, comes from the incoming image
        for tag in tags:
            # Pose of the apriltag in the camera coordinate system
            ...
            # Publish the frame of the estimated apriltag location (as
            # observed by the camera)
            self._tf_bcaster.sendTransform(
                p.tolist(),
                q.tolist(),
                msg.header.stamp,  # Time
                "tag/{:s}".format(str(tag.tag_id)), # Child frame
        ...
        # Render visualization (if needed, i.e. if anyone is listening)
    ...
    def _img_cb(self, msg):
        ...
        if not self._detection_reminder.is_time(frequency=self.detection_freq.value):
            # rospy.loginfo("not time for detecting")
            return

        # make sure we are still running
        if self.is_shutdown:
            rospy.loginfo("we're shut down!")
            return
        ...
        for i in range(self.ndetectors):
            if self._tasks[i] is None or self._tasks[i].done():
                ...
                # rospy.loginfo("submitting to worker!")
...
if __name__ == "__main__":
    rospy.loginfo("april_tag_detector initializing...")
